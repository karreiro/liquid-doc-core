...
async function loadParsers() {
    try {
        ...
        try {
            ...
            console.warn("⚠️ npm package not available, trying Node.js minified parser...");
            // Fall back to Node.js compatible minified parser
            const { toLiquidHtmlAST: nodeParser } = await import('./ohm-parser-node.js');
...
function testFixture(fixture, iterations = 100) {
    ...
    const benchmarks = [
        [useMinifiedParser ? "Node.js Minified Ohm.js" : "Ohm.js", () => toLiquidHtmlAST(ohmInput)]
    ];
...
function aggregateResults(allResults) {
    ...
    if (ohmTimes.length > 0) {
        ...
        console.log(`🔍 ${useMinifiedParser ? 'Node.js Minified ' : ''}Ohm.js average across all fixtures: ${ohmAvg.toFixed(4)} ms`);
    }
    ...
    const summary = {
        ...
        ohmAverage: ohmTimes.length > 0 ? ohmTimes.reduce((a, b) => a + b, 0) / ohmTimes.length : null,
        pestAverage: pestTimes.length > 0 ? pestTimes.reduce((a, b) => a + b, 0) / pestTimes.length : null,
        speedup: ohmTimes.length > 0 && pestTimes.length > 0 ?
            (ohmTimes.reduce((a, b) => a + b, 0) / ohmTimes.length) / (pestTimes.reduce((a, b) => a + b, 0) / pestTimes.length) : null
    };
